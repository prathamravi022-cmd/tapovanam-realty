import { c as createLucideIcon, r as reactExports, B as jsxRuntimeExports, Y as Presence, Z as Primitive, _ as useControllableState, $ as useComposedRefs, a0 as composeEventHandlers, a1 as createContextScope, D as cn, a2 as commonjsGlobal, a3 as getDefaultExportFromCjs, a4 as clsx, a5 as React2, a6 as invariant, O as useActor, a7 as useInternetIdentity, F as useQueryClient, Q as useQuery, R as createActor, P as PropertyStatus, C as motion, L as Link, K as Button, a8 as Building2, a9 as ue } from "./index-D6obxqBN.js";
...
import { u as usePrevious, a as useSize, T as Trash2 } from "./index-IHAb3OcJ.js";
import { C as Check, S as Star } from "./star-Dhud6W4L.js";
import { S as Skeleton } from "./skeleton-24zo03YF.js";
import { a as formatArea, f as formatPrice, P as PropertyStatusBadge, L as LegalStatusBadge } from "./StatusBadge-cIrDUTo3.js";
import { u as useProperties, a as useDeleteProperty, b as useSetPropertyStatus } from "./useProperties-SsI244bB.js";
import { P as Plus } from "./plus-4kNKOwcO.js";
import { H as HardHat } from "./hard-hat-BZ9LXJGF.js";
import { E as Eye } from "./eye-Be1RDvn9.js";
...
var Sector = function Sector2(sectorProps) {
  var props = _objectSpread$o(_objectSpread$o({}, defaultProps$2), sectorProps);
...
function useAdmin() {
  ...
  const { login, clear, identity: identity3, loginStatus } = useInternetIdentity();
  const queryClient = useQueryClient();
  const isAuthenticated = !!identity3;
...
function AnimatedCounter({ value }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: displayed });
}
function StatCard({
  label,
  value,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    motion.div,
    {
      ...
      className: `relative overflow-hidden rounded-2xl border border-border bg-card p-5 shadow-card ${glowClass ?? ""}`,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `text-3xl font-display font-bold ${colorClass}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(AnimatedCounter, { value }) })
        ] })
      ]
    }
  );
...
function GlassTooltip({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-light rounded-xl px-3 py-2 text-xs shadow-elevation", children: [
    label && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium text-foreground mb-1", children: label }),
    payload.map((entry) => /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-muted-foreground", children: [
      entry.name,
      ":",
      " ",
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { className: "text-foreground", children: entry.value })
    ] }, entry.name))
  ] });
}
function AdminDashboardPage() {
  ...
  const [bulkDeleting, setBulkDeleting] = reactExports.useState(false);
  const total = (properties == null ? void 0 : properties.length) ?? 0;
  const available = (properties == null ? void 0 : properties.filter((p2) => p2.status === PropertyStatus.available).length) ?? 0;
  const sold = total - available;
  const constructionCount = (properties == null ? void 0 : properties.filter((p2) => p2.isConstructionSite).length) ?? 0;
  const locationData = reactExports.useMemo(() => {
    if (!properties) return [];
    const counts = {};
    for (const p2 of properties) {
      const loc = p2.locationName.split(",")[0].trim();
      counts[loc] = (counts[loc] ?? 0) + 1;
    }
    return Object.entries(counts).sort((a2, b2) => b2[1] - a2[1]).slice(0, 6).map(([name, count]) => ({ name, count }));
  }, [properties]);
  const monthlyData = reactExports.useMemo(() => {
    if (!properties) return [];
    const counts = {};
    for (const p2 of properties) {
      const date2 = new Date(Number(p2.dateAdded / BigInt(1e6)));
      const key = `${date2.getFullYear()}-${String(date2.getMonth() + 1).padStart(2, "0")}`;
      counts[key] = (counts[key] ?? 0) + 1;
    }
    return Object.entries(counts).sort().slice(-6).map(([month, count]) => ({ month: month.slice(5), count }));
  }, [properties]);
  ...
  const sorted = reactExports.useMemo(() => {
    if (!properties) return [];
    return [...properties].sort((a2, b2) => {
      let diff = 0;
      if (sortKey === "title") diff = a2.title.localeCompare(b2.title);
      else if (sortKey === "price") diff = Number(a2.price - b2.price);
      else if (sortKey === "areaSizeSqFt")
        diff = a2.areaSizeSqFt - b2.areaSizeSqFt;
      else diff = Number(a2.dateAdded - b2.dateAdded);
      return sortAsc ? diff : -diff;
    });
  }, [properties, sortKey, sortAsc]);
  const mostRecent = reactExports.useMemo(
    () => properties ? [...properties].sort((a2, b2) => Number(b2.dateAdded - a2.dateAdded))[0] : void 0,
    [properties]
  );
  const thisMonthCount = reactExports.useMemo(() => {
    if (!properties) return 0;
    const now2 = /* @__PURE__ */ new Date();
    return properties.filter((p2) => {
      const d2 = new Date(Number(p2.dateAdded / BigInt(1e6)));
  ...
  const handleToggleStatus = async (p2) => {
    const newStatus = p2.status === PropertyStatus.available ? PropertyStatus.sold : PropertyStatus.available;
    try {
  ...
  const handleBulkDelete = async () => {
    ...
    if (failed === 0) ue.success(`Deleted ${count} properties`);
    else ue.error(`${failed} deletions failed`);
  };
  const toggleSort = (key) => {
    if (sortKey === key) setSortAsc((v2) => !v2);
    else {
      setSortKey(key);
      setSortAsc(true);
    }
  ...
  const toggleSelectAll = () => {
    if (selected.size === sorted.length) setSelected(/* @__PURE__ */ new Set());
    else setSelected(new Set(sorted.map((p2) => p2.propertyId)));
  };
  if (authLoading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto px-4 py-12", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Skeleton, { className: "h-10 w-64 mb-6" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-4 mb-8", children: [1, 2, 3, 4].map((i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Skeleton, { className: "h-28 w-full rounded-2xl" }, i)) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-3", children: [1, 2, 3].map((i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Skeleton, { className: "h-16 w-full" }, i)) })
    ] });
  }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto px-4 py-8", children: [
      ...
      motion.div,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-display text-2xl font-bold text-foreground", children: "Property Dashboard" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm mt-0.5", children: "Manage all land and plot listings" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/admin/property/new", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
            Button,
            {
    ...
    properties && properties.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
      motion.div,
      {
        ...
        children: [
          ...
          },
          {
            icon: Star,
            label: "Most recent",
            value: (mostRecent == null ? void 0 : mostRecent.title) ?? "—",
        ...
        ].map(({ icon: Icon, label, value, ocid }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            className: "flex items-center gap-3 bg-muted/40 rounded-xl px-4 py-3 border border-border",
            "data-ocid": ocid,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-7 h-7 rounded-lg bg-card flex items-center justify-center shadow-card shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Icon, { className: "w-3.5 h-3.5 text-primary" }) }),
    ...
    properties && properties.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
      motion.section,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-light rounded-2xl p-5 border border-border shadow-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "font-display font-semibold text-sm text-foreground mb-4 flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-1.5 h-4 rounded-full bg-primary inline-block" }),
              "Availability Split"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 160, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                Pie,
                {
                  ...
                  children: donutData.map((_, i) => (
                    // biome-ignore lint/suspicious/noArrayIndexKey: stable
                    /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: DONUT_COLORS[i] }, i)
                  ))
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { content: /* @__PURE__ */ jsxRuntimeExports.jsx(GlassTooltip, {}) })
            ] }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center justify-center gap-4 mt-2", children: donutData.map((d2, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5 text-xs", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-light rounded-2xl p-5 border border-border shadow-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "font-display font-semibold text-sm text-foreground mb-4 flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-1.5 h-4 rounded-full bg-secondary inline-block" }),
              "By Location"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 160, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BarChart, { data: locationData, margin: { left: -20 }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                XAxis,
                {
                  dataKey: "name",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-light rounded-2xl p-5 border border-border shadow-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "font-display font-semibold text-sm text-foreground mb-4 flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-1.5 h-4 rounded-full bg-accent inline-block" }),
              "Monthly Additions"
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: 160, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(AreaChart, { data: monthlyData, margin: { left: -20 }, children: [
    ...
    selected.size > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
      motion.div,
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-sm font-medium text-foreground flex-1", children: [
            selected.size,
            " selected"
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialog, { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialogContent, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialogHeader, { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(AlertDialogTitle, { children: "Bulk Delete" }),
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialogFooter, { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(AlertDialogCancel, { "data-ocid": "dashboard.bulk_cancel_button", children: "Cancel" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  AlertDialogAction,
                  {
    ...
    propsLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-3", children: [1, 2, 3, 4].map((i) => /* @__PURE__ */ jsxRuntimeExports.jsx(Skeleton, { className: "h-20 w-full" }, i)) }) : !properties || properties.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
      motion.div,
      {
        initial: { opacity: 0 },
        animate: { opacity: 1 },
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Building2, { className: "w-12 h-12 text-muted-foreground mx-auto mb-4" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-display font-semibold text-foreground mb-2", children: "No properties yet" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground text-sm mb-5", children: "Add your first land or plot listing to get started." }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/admin/property/new", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Button, { variant: "outline", className: "gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { className: "w-4 h-4" }),
            " Add First Property"
          ] }) })
        ]
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "hidden md:block overflow-x-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full text-sm", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-b border-border bg-muted/40", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-medium text-muted-foreground w-16", children: "Img" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-medium text-muted-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-medium text-muted-foreground", children: "Location" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-right px-4 py-3 font-medium text-muted-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  type: "button",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left px-4 py-3 font-medium text-muted-foreground", children: "Legal" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-right px-4 py-3 font-medium text-muted-foreground", children: "Actions" })
            ] }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { className: "divide-y divide-border", children: sorted.map((p2, idx) => {
              var _a;
              return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "tr",
                {
                  className: `hover:bg-muted/20 transition-colors ${selected.has(p2.propertyId) ? "bg-primary/5" : ""}`,
                  "data-ocid": `dashboard.property_row.${idx + 1}`,
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      Checkbox,
                      {
                        checked: selected.has(p2.propertyId),
                        onCheckedChange: () => toggleSelect(p2.propertyId),
                        "aria-label": `Select ${p2.title}`,
                        "data-ocid": `dashboard.row_checkbox.${idx + 1}`
                      }
                    ) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: ((_a = p2.images) == null ? void 0 : _a[0]) ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "img",
                      {
                        src: p2.images[0].getDirectURL(),
                        alt: p2.title,
                        className: "w-12 h-10 object-cover rounded-lg border border-border"
                      }
                    ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-12 h-10 rounded-lg bg-muted flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Building2, { className: "w-4 h-4 text-muted-foreground" }) }) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "px-4 py-3", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "font-medium text-foreground truncate max-w-[160px] flex items-center gap-1.5", children: [
                        p2.isConstructionSite && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "span",
                          {
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground", children: formatArea(p2.areaSizeSqFt) })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-muted-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate max-w-[120px] block", children: p2.locationName }) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right tabular-nums font-medium", children: formatPrice(p2.price) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        type: "button",
                        onClick: () => handleToggleStatus(p2),
                        disabled: setStatus.isPending,
                        title: `Toggle to ${p2.status === PropertyStatus.available ? "Sold" : "Available"}`,
                        className: "transition-smooth",
                        "data-ocid": `dashboard.toggle_status.${idx + 1}`,
                        children: /* @__PURE__ */ jsxRuntimeExports.jsx(PropertyStatusBadge, { status: p2.status })
                      }
                    ) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LegalStatusBadge, { status: p2.legalStatus }) }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-end gap-1", children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialog, { children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialogContent, { children: [
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsxs(AlertDialogFooter, { children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx(AlertDialogCancel, { "data-ocid": "dashboard.cancel_button", children: "Cancel" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx(
                              AlertDialogAction,
                              {
                                className: "bg-destructive text-destructive-foreground hover:bg-destructive/90",
                                onClick: () => handleDelete(p2.propertyId, p2.title),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "md:hidden divide-y divide-border", children: sorted.map((p2, idx) => {
            var _a;
            return /* @__PURE__ */ jsxRuntimeExports.jsx(
              motion.div,
              {
                ...
                className: `p-4 ${selected.has(p2.propertyId) ? "bg-primary/5" : ""}`,
                ...
                children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-3", children: [
                  ...
                  ((_a = p2.images) == null ? void 0 : _a[0]) ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "img",
                    {
                      src: p2.images[0].getDirectURL(),
                      alt: p2.title,
                  ...
                  ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-14 rounded-lg bg-muted flex items-center justify-center shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Building2, { className: "w-5 h-5 text-muted-foreground" }) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-medium text-foreground truncate", children: p2.title }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground mt-0.5", children: p2.locationName }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mt-1.5 flex-wrap", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(PropertyStatusBadge, { status: p2.status }),
                      p2.isConstructionSite && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "inline-flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx(HardHat, { className: "w-2.5 h-2.5" }),
                        "Construction"
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs font-medium text-foreground", children: formatPrice(p2.price) })
                    ] })
                  ] }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 py-2.5 border-t border-border bg-muted/20 text-xs text-muted-foreground", children: [
            total,
            " ",
